...
def get_frame_det(dets_all, frame):
    ...
    other_array = dets_all[
        dets_all[:, 0] == frame, 1:7
...
#         assert (
#             id_tmp not in data_dict[seq_id][frame_id]
#         ), "error, each ID should not be highlighted twice"
...
def save_affinity(affi_data, save_path):
    ...
    num_digit += (
        decimals + 1
    )  # meaning that we want to preserve the dot plus the decimals
