...
def vis_obj(
    ...
    img, draw = draw_box3d_image(
        img, obj_pts_2d, hw, color=color_tmp, thickness=thickness
    )
    ...
    if err_type is not None:
        ...
        xmin = np.min(obj_pts_2d[:, 0])
        xmax = np.max(obj_pts_2d[:, 0])
        ymin = np.min(obj_pts_2d[:, 1])
        ymax = np.max(obj_pts_2d[:, 1])
        ...
        cv2.rectangle(
            img,
            (pos_x, pos_y - text_h - 5),
            (pos_x + text_w, pos_y + 5),
            (255, 255, 255),
            -1,
        )  # add white background
        img = cv2.putText(
            img, err_type, (pos_x, pos_y), font, font_scale, (255, 0, 0), font_thickness
        )

    return img


def vis_image_with_obj(
    img,
    obj_res,
    obj_gt,
    calib,
        ...
        if depth >= 2:  # check in front of camera
            ...
            if color_type == "trk":
                ...
                if id_hl is not None and obj.id in id_hl:
                    ...
                    color_id = (
                        obj.id * 9
                    )  # some magic number to scale up the id so that nearby ID does not have similar color
